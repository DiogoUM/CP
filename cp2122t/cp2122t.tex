...
Para a resolução deste problema desenvolveu-se o seguinte diagrama para a respresentação do catamorfismo de listas \ensuremath{\Varid{propagate}}.
\begin{eqnarray*}
\xymatrix@C=3cm@R=1.7cm{
...
\end{eqnarray*}
Aqui é possível verificar que é necessária a aplicação de \ensuremath{\Varid{bflip}} em todos os elementos da lista de \ensuremath{\Conid{Bits}}. Com estes dados foi definida a função (g f).
...
\end{hscode}\resethooks
\\
No \ensuremath{\Varid{propagate3}} o raciocínio usado foi similar ao do \ensuremath{\Varid{propagate}}, sendo que neste é realizado o \ensuremath{\Varid{bflip3}} o que implica a execução da função \ensuremath{\mathsf{fmap}} e \ensuremath{v_3 } de modo a votar no bit mais frequente para uma maior probabilidade da resposta certa.
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{4}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
